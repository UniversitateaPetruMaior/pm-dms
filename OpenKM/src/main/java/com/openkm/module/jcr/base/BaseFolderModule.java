...
public class BaseFolderModule {
	...
	public static Node create(Session session, Node parentNode, String name) throws 
		...
		folderNode.setProperty(Property.KEYWORDS, new String[]{});
		folderNode.setProperty(Property.CATEGORIES, new String[]{}, PropertyType.REFERENCE);
	...
	public static Folder getProperties(Session session, Node fldNode) throws 
		...
		Value[] keywords = fldNode.getProperty(Property.KEYWORDS).getValues();

		for (int i=0; i<keywords.length; i++) {
			keywordsSet.add(keywords[i].getString());
		}

		fld.setKeywords(keywordsSet);
		
		// Get folder categories
		Set<Folder> categoriesSet = new HashSet<Folder>();
		Value[] categories = fldNode.getProperty(Property.CATEGORIES).getValues();
		...
		fld.setCategories(categoriesSet);
	 ...
	 * TODO: Is this necessary? The access manager should prevent this and
	 * make the core thrown an exception. 
	 */
	...
	public static void purge(Session session, Node fldNode) throws VersionException, 
		...
		log.debug("purge({}, {})", new Object[] { session, fldNode.getPath() });
