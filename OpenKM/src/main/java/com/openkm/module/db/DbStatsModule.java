...
public class DbStatsModule implements StatsModule {
	...
	public StatsInfo getDocumentsSizeByContext(String token) throws RepositoryException, DatabaseException {
		...
		try {
			...
			long taxonomyDocumentSize = NodeDocumentDAO.getInstance().getSubtreeSize("/" + Repository.ROOT);
			long personalDocumentSize = NodeDocumentDAO.getInstance().getSubtreeSize("/" + Repository.PERSONAL);
			long templatesDocumentSize = NodeDocumentDAO.getInstance().getSubtreeSize("/" + Repository.TEMPLATES);
			long trashDocumentSize = NodeDocumentDAO.getInstance().getSubtreeSize("/" + Repository.TRASH);
