		...
		while (thisDiff != null) {
			if (thisDiff.operation == Operation.EQUAL) {
				...
				if (lastequality != null
						&& ((pre_ins && pre_del && post_ins && post_del) || ((lastequality.length() < diffEditCost / 2) && ((pre_ins ? 1
	...
	protected int matchBitap(String text, String pattern, int loc) {
		...
		for (int d = 0; d < pattern.length(); d++) {
			...
			while (bin_min < bin_mid) {
				if (matchBitapScore(d, loc + bin_mid, loc, pattern) <= score_threshold) {
					bin_min = bin_mid;
				} else {
					bin_max = bin_mid;
				}
	...
	public Object[] patchApply(LinkedList<Patch> patches, String text) {
		...
		for (Patch aPatch : patches) {
			...
			if (text1.length() > this.matchMaxBits) {
				...
				// a monster delete.
				start_loc = matchMain(text, text1.substring(0, this.matchMaxBits), expected_loc);
				if (start_loc != -1) {
					end_loc = matchMain(text, text1.substring(text1.length() - this.matchMaxBits), expected_loc + text1.length()
							- this.matchMaxBits);
			...
			if (start_loc == -1) {
				...
				if (end_loc == -1) {
					...
					text2 = text.substring(start_loc, Math.min(end_loc + this.matchMaxBits, text.length()));
				}
				if (text1.equals(text2)) {
					// Perfect match, just shove the replacement text in.
					text = text.substring(0, start_loc) + diffText2(aPatch.diffs) + text.substring(start_loc + text1.length());
				} else {
							...
							if (aDiff.operation != Operation.EQUAL) {
								...
								if (aDiff.operation == Operation.INSERT) {
									...
									text = text.substring(0, start_loc + index2)
											+ text.substring(start_loc + diffXIndex(diffs, index1 + aDiff.text.length()));
								}
							}
							if (aDiff.operation != Operation.DELETE) {
				...
				while (!bigpatch.diffs.isEmpty() && patch.length1 < patch_size - patchMargin) {
					...
					if (diff_type == Operation.INSERT) {
						...
						empty = false;
							...
							&& patch.diffs.getFirst().operation == Operation.EQUAL && diff_text.length() > 2 * patch_size) {
						...
						diff_text = diff_text.substring(0, Math.min(diff_text.length(), patch_size - patch.length1 - patchMargin));
						patch.length1 += diff_text.length();
						start1 += diff_text.length();
