...
public class BenchmarkServlet extends BaseServlet {
	...
	private static final String[][] breadcrumb = new String[][] {
		...
		new String[] { "benchmark.jsp", "Benchmark" }
	};
	 ...
	 */
	private void okmImport(HttpServletRequest request, HttpServletResponse response, String base) throws IOException {
		...
		try {
			...
			for (int i = 0; i < times; i++) {
				...
				OKMFolder.getInstance().create(null, fld);
				ImpExpStats stats = RepositoryImporter.importDocuments(null, dir, fld.getPath(), false, false, false, out,
	 ...
	 */
	private void okmApiHighGenerate(HttpServletRequest request, HttpServletResponse response, String base)
			throws IOException {
		...
		Benchmark bm = null;
		response.setContentType(MimeTypeConfig.MIME_HTML);
		header(out, "OpenKM generate documents (API HIGH)", breadcrumb);
		out.flush();
		...
		try {
			...
			for (int i = 0; i < maxIterations; i++) {
				...
				out.println("<tr><th>Date</th><th>Partial time</th><th>Partial miliseconds</th><th>Total folders</th><th>Total documents</th><th>Total size</th></tr>");
				out.flush();
				...
				out.println("<table class=\"results\" width=\"80%\">");
				out.println("<tr><th>Date</th><th>Partial time</th><th>Total folders</th><th>Total documents</th><th>Total size</th></tr>");
		...
		// Activity log
		UserActivity.log(request.getRemoteUser(), "ADMIN_BENCHMARK_OKM_API_HIGH", null, null,
				"Size: " + FormatUtil.formatSize(bm == null ? 0 : bm.getTotalSize()) + ", Folders: " + (bm == null? 0 : bm.getTotalFolders())
						+ ", Documents: " + (bm == null? 0 : bm.getTotalDocuments()) + ", Time: " + FormatUtil.formatSeconds(elapse));
	 ...
	 */
	private void jcrGenerate(HttpServletRequest request, HttpServletResponse response, String base) throws IOException {
