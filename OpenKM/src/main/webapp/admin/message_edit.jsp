...
<link rel="stylesheet" type="text/css"
	href="../js/cleditor/jquery.cleditor.css" />
...
<body>
	...
	<u:constantsMap
		className="com.openkm.frontend.client.bean.GWTUINotification"
		var="GWTUINotification" />
	<c:choose>
		<c:when test="${isAdmin}">
			...
			<ul id="breadcrumb">
				...
				<li class="path"><a href="${messageList}">Message queue</a></li>
			...
			<form action="LoggedUsers">
				<input type="hidden" name="action" value="${action}" /> <input
					...
					type="hidden" name="me_id" value="${me.id}" />
				<table class="form" width="372px">
					<tr>
						<td>Action</td>
						<td width="100%"><select name="me_action" id="me_action">
								<option value="">-</option>
								<c:choose>
									<c:when test="${me.action == GWTUINotification.ACTION_LOGOUT}">
										<option value="${GWTUINotification.ACTION_LOGOUT}"
											selected="selected">Logout</option>
									</c:when>
									<c:otherwise>
					...
					<tr>
						<td>Type</td>
						<td width="100%"><select name="me_type" id="me_type"
							class=":integer :required :only_on_blur">
								<option value="">-</option>
								<c:choose>
									<c:when test="${me.type == GWTUINotification.TYPE_TEMPORAL}">
										<option value="${GWTUINotification.TYPE_TEMPORAL}"
											selected="selected">Temporal</option>
									</c:when>
									<c:otherwise>
								...
								<c:choose>
									<c:when test="${me.type == GWTUINotification.TYPE_PERMANENT}">
										<option value="${GWTUINotification.TYPE_PERMANENT}"
											selected="selected">Permanent</option>
									</c:when>
									<c:otherwise>
					...
					<tr>
						<td colspan="2"><textarea class=":required :only_on_blur"
								name="me_message" id="me_message" rows="10" cols="60">${me.message}</textarea>
						</td>
					</tr>
					<tr>
						<td colspan="2" align="right"><input type="button"
							...
							class="noButton" /> <c:choose>
								<c:when test="${action == 'messageCreate'}">
									<input type="submit" value="Create" class="yesButton" />
								</c:when>
								<c:when test="${action == 'messageEdit'}">
