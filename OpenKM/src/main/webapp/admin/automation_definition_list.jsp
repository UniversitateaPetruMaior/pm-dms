		...
		<c:when test="${isAdmin}">
			...
			<table class="results" width="70%">
					...
					<tr class="fuzzy">
						<td colspan="7" align="left">
							<table id="info" style="white-space: nowrap;" width="175px">
								<tr>
									<td valign="top"><b>Automation rule</b> ${ar.name}</td>
									<td width="100%" align="right"><a id="show_hide" href="#"><img
											title="Show / Hide" alt="Show / Hide"
											src="img/action/examine.png" /></a></td>
								</tr>
								<tr>
									<td>
										<div id="extra_info">
					...
					<td colspan="6">
						<table>
							<tr>
								...
								<td><select id="ma_id_validation">
										<option value="">-</option>
										<c:forEach var="mv" items="${metadaValidations}">
											<option value="${mv.id}">${mv.name}</option>
										</c:forEach>
								</select></td>
								<td><img id="addValidation" src="img/action/new.png"
									style="cursor: pointer; cursor: hand;" alt="New validation"
									title="New validation" /></td>
							</tr>
						</table>
				...
				<c:forEach var="validation" items="${ar.validations}"
					...
					<tr class="${row.index % 2 == 0 ? 'even' : 'odd'}">
		    	  	...
		    	  	<c:set value="${mv.description00}" var="description00"></c:set>
						...
						<td><c:if
								test="${validation.params.size() > 0 && description00 !=null && description00 ne ''}">
								<c:set value="${validation.params.get(0)}" var="param00"></c:set>
              	  ${description00}:${param00}
              	</c:if></td>
						<td><c:if
								test="${validation.params.size() > 1 && description01 !=null && description01 ne ''}">
								<c:set value="${validation.params.get(1)}" var="param01"></c:set>
              	  ${description01}: ${param01}
              	</c:if></td>
						<td align="center"><a
							href="javascript:editValidation(${validation.type}, ${validation.id})"><img
								src="img/action/edit.png" alt="Edit" title="Edit" /></a> &nbsp; <a
							href="javascript:deleteValidation(${validation.type}, ${validation.id})"><img
								src="img/action/delete.png" alt="Delete" title="Delete" /></a></td>
					</tr>
				</c:forEach>
				<tr>
					<td colspan="6">
						<table>
							<tr>
								...
								<td><select id="ma_id_action">
										<option value="">-</option>
										<c:forEach var="ma" items="${metadaActions}">
											<option value="${ma.id}">${ma.name}</option>
										</c:forEach>
								</select></td>
								<td><img id="addAction" src="img/action/new.png"
									style="cursor: pointer; cursor: hand;" alt="New action"
									title="New action" /></td>
							</tr>
						</table>
				...
				<c:forEach var="action" items="${ar.actions}" varStatus="row">
					...
					<tr class="${row.index % 2 == 0 ? 'even' : 'odd'}">
						<td>${action.order}</td>
						<td><c:forEach var="ma" items="${metadaActions}">
		    	  	...
		    	  	<c:if test="${ma.name == 'ExecuteScripting'}">
						...
						<td><c:if
								test="${action.params.size() > 0 && description00 !=null && description00 ne ''}">
								<c:set value="${action.params.get(0)}" var="param00"></c:set>
								<c:if test="${showParam0}">${description00}:${param00}</c:if>
							</c:if></td>
						<td><c:if
								test="${action.params.size() > 1 && description01 !=null && description01 ne ''}">
								<c:set value="${action.params.get(1)}" var="param01"></c:set>
              	  ${description01}: ${param01}
              	</c:if></td>
						<td align="center"><a
							href="javascript:editAction(${action.type}, ${action.id})"><img
								src="img/action/edit.png" alt="Edit" title="Edit" /></a> &nbsp; <a
							href="javascript:deleteAction(${action.type}, ${action.id})"><img
								src="img/action/delete.png" alt="Delete" title="Delete" /></a></td>
					</tr>
				</c:forEach>
			</table>
