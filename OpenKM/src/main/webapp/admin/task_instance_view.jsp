	...
	<c:choose>
		<c:when test="${isAdmin}">
			<c:url value="Workflow" var="urlProcessDefinitionView">
				...
				<c:param name="pdid"
					value="${taskInstance.processInstance.processDefinition.id}" />
			...
			<ul id="breadcrumb">
				...
				<li class="path"><a href="${urlProcessDefinitionView}">Process
						definition</a></li>
				<li class="path"><a href="${urlProcessInstanceView}">Process
						instance</a></li>
				<li class="path">Task instance</li>
				<li class="action"><a href="${urlTaskInstanceView}"> <img
			...
			<table class="results" width="90%">
				...
				<tr class="even">
					...
					<td><b> <c:choose>
								<c:when
									test="${taksInstance.end != null && taksInstance.suspended}">Ended (suspended)</c:when>
								<c:when
									test="${taksInstance.end != null && !taksInstance.suspended}">Ended</c:when>
								<c:when
									test="${taksInstance.end == null && taskInstance.start == null && taksInstance.suspended}">Not started (suspended)</c:when>
								<c:when
									test="${taksInstance.end == null && taskInstance.start == null && !taksInstance.suspended}">Not started</c:when>
								<c:when
									test="${taksInstance.end == null && taskInstance.start != null && taksInstance.suspended}">Suspended</c:when>
								<c:when
									test="${taksInstance.end == null && taskInstance.start != null && !taksInstance.suspended}">Running</c:when>
							</c:choose>
					</b></td>
					...
					<td><a href="${urlProcessDefinitionView}">
							${taskInstance.processInstance.processDefinition.name}
							v${taskInstance.processInstance.processDefinition.version} </a></td>
			...
			<table class="results" width="90%">
				...
				<c:forEach var="cmt" items="${taskInstance.comments}"
					varStatus="row">
					<tr class="${row.index % 2 == 0 ? 'even' : 'odd'}">
						<td>${cmt.actorId}</td>
			...
			<table class="results" width="90%">
				...
				<c:forEach var="var" items="${variables}" varStatus="row">
					...
					<tr class="${row.index % 2 == 0 ? 'even' : 'odd'}">
						<td>${var.key}</td>
						<td>${var.value}</td>
						<td><a href="${urlTaskInstanceVariableDelete}"><img
			...
			<table class="results" width="90%">
				...
				<c:forEach var="tr" items="${taskInstance.availableTransitions}"
					varStatus="row">
					...
					<tr class="${row.index % 2 == 0 ? 'even' : 'odd'}">
						...
						<td>${tr.name}</td>
						<td>${tr.to}</td>
						<td><c:if test="${!taskInstance.suspended}">
								<a href="${urlTaskInstanceEnd}"><img
									src="img/action/end.png" alt="End task" title="End task" /></a>
							</c:if></td>
					</tr>
				</c:forEach>
			</table>
