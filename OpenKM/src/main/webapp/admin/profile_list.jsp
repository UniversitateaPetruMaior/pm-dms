		...
		<c:when test="${isAdmin}">
			...
			<table class="results" width="40%">
				<tr>
					...
					<th width="75px"><c:url value="Profile" var="urlCreate">
							<c:param name="action" value="create" />
						</c:url> <a href="${urlCreate}"><img src="img/action/new.png"
							alt="New profile" title="New profile" /></a></th>
				</tr>
				<c:forEach var="prf" items="${userProfiles}" varStatus="row">
					...
					<tr class="${row.index % 2 == 0 ? 'even' : 'odd'}">
						...
						<td align="center"><a href="${urlEdit}"><img
								src="img/action/edit.png" alt="Edit" title="Edit" /></a> <c:if
								test="${prf.id != 1}">
                &nbsp;
                <a href="${urlDelete}"><img
									src="img/action/delete.png" alt="Delete" title="Delete" /></a>
							</c:if> &nbsp; <a href="${urlClone}"><img src="img/action/clone.png"
								alt="Clone" title="Clone" /></a></td>
					</tr>
				</c:forEach>
			</table>
		</c:when>
