...
<link rel="stylesheet" type="text/css"
	href="css/style.css?v=%{TIMESTAMP}%" />
		...
		<c:when test="${isAdmin}">
			...
			<form action="Omr" method="post" enctype="multipart/form-data"
				...
				<input type="hidden" name="action" value="${action}" /> <input
					type="hidden" name="om_id" value="${om.id}" />
				<table class="form" width="425px">
					<tr>
						<td colspan="2">
							<div id="alert" class="ok" style="text-align: center;">The
								operation may take several minutes, please be patient...</div>
						</td>
					</tr>
					<tr>
						<td nowrap="nowrap">Template name</td>
						<td><input class=":required :only_on_blur" name="om_name"
							value="${om.name}" /></td>
					</tr>
					<tr>
						<td>Template</td>
						<td><c:if
								test="${om.templateFileName != null && om.templateFileName ne ''}">
					...
					<tr>
						<td>Properties</td>
						<td><select id="om_properties" name="om_properties"
							data-placeholder="Select property" multiple="multiple"
							style="width: 250px">
					...
					<c:if test="${om.ascFileName != null && om.ascFileName ne ''}">
						...
						<tr>
							<td>Asc</td>
							<td><a href="${urlDownload}&type=2">${om.ascFileName}</a> <c:if
									test="${action ne 'delete'}">
									<a href="${urlEditAsc}"><img src="img/action/edit.png"
										alt="Edit" title="Edit" /></a>
								</c:if></td>
						</tr>
					</c:if>
					<c:if
						test="${om.configFileName != null && om.configFileName ne ''}">
						<tr>
							<td>Config</td>
							<td><a href="${urlDownload}&type=3">${om.configFileName}</a>
							</td>
						</tr>
					</c:if>
					<c:if test="${om.ascFileName != null && om.ascFileName ne ''}">
						...
						<tr>
							...
							<td><c:choose>
									<c:when
										test="${om.fieldsFileName != null && om.fieldsFileName ne ''}">
								...
								</c:choose> <c:if test="${action ne 'delete' }">
									<a href="${urlEditFields}"><img src="img/action/edit.png"
										alt="Edit" title="Edit" /></a>
								</c:if></td>
						</tr>
					</c:if>
					...
					<tr>
						<td colspan="2" align="right"><c:url value="Omr"
								var="urlCancel">
								<c:param name="action" value="userList" />
							</c:url> <input type="button"
							onclick="javascript:window.location.href='${urlCancel}'"
							value="Cancel" class="noButton" /> <c:choose>
								<c:when test="${action == 'create'}">
