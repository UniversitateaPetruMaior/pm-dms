...
<link rel="stylesheet" type="text/css"
	href="js/codemirror/mode/plsql/plsql.css" />
<style type="text/css">
.CodeMirror {
	width: 700px;
	height: 300px;
...
.results .cellEditing input[type=text] {
	width: 100%;
	border: 0;
...
<body>
	...
	<c:choose>
		<c:when test="${isAdmin}">
			<ul id="breadcrumb">
				<li class="path"><a href="DatabaseQuery">Database query</a></li>
			</ul>
			<br />
			<table class="form">
				<tr>
					<td>
						<form action="DatabaseQuery" method="post"
							enctype="multipart/form-data">
							<table>
								<tr>
									<td colspan="2"><textarea cols="80" rows="25" name="qs"
											id="qs">${qs}</textarea></td>
								</tr>
								<tr>
									<td align="left">
										<div id="divTables">
											Table <select id="tables" name="tables">
												<option></option>
										...
										<div id="divVTables">
											Metatable <select id="vtables" name="vtables">
												<option></option>
										...
										<div id="divShowSql">
											<table cellpadding="0" cellspacing="0">
												<tr>
													...
													<td><c:choose>
															<c:when test="${showSql}">
																<input name="showSql" type="checkbox" checked="checked"
									...
									<td align="right">Type <select name="type" id="type">
											<option></option>
											<c:choose>
									...
									</select> <input type="submit" value="Execute" class="executeButton" />
									</td>
								</tr>
							</table>
						</form>
							...
							<table>
								<tr>
									<td><input class=":required :only_on_blur" type="file"
										name="sql-file" /></td>
									<td><input type="submit" value="Import SQL script"
										class="loadButton" /></td>
								</tr>
							</table>
						</form>
				...
				<c:choose>
					...
					<c:otherwise>
						<table class="results" width="95%">
							...
							<c:forEach var="result" items="${gResult.results}">
								<tr class="${row.index % 2 == 0 ? 'even' : 'odd'}">
