			...
			<form action="Config" method="post" enctype="multipart/form-data">
				...
				<table class="form" width="425px">
					<tr>
						...
						<td><c:choose>
								<c:when test="${action != 'create'}">
									<input size="50" class=":required :only_on_blur" name="cfg_key"
										value="${cfg.key}" readonly="readonly" />
								</c:when>
								<c:otherwise>
									<input size="50" class=":required :only_on_blur" name="cfg_key"
										value="" />
					...
					<tr>
						...
						<td><c:choose>
								<c:when
									test="${cfg.type == 'string' || cfg.type == 'integer' || cfg.type == 'long'}">
								...
								<c:when test="${cfg.type == 'file'}">
									<c:choose>
										<c:when test="${action == 'create'}">
											<input class=":required :only_on_blur" type="file"
												name="file" />
										</c:when>
										<c:otherwise>
