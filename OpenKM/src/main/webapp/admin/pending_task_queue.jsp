...
<body>
	<u:constantsMap className="com.openkm.dao.bean.PendingTask"
		var="PendingTask" />
	<c:set var="isAdmin"><%=BaseServlet.isAdmin(request)%></c:set>
	<c:choose>
		<c:when test="${isAdmin}">
			<ul id="breadcrumb">
				<li class="path"><a href="PendingTaskQueue">Pending task
						queue</a></li>
				<li class="action"><a href="stats.jsp"> <img
						src="img/action/generic.png" alt="Generic" title="Generic"
						style="vertical-align: middle;" /> Statistics
				</a></li>
				<li class="action"><a href="PendingTaskQueue"> <img
				...
				<tbody>
					<c:forEach var="task" items="${pendingTasks}" varStatus="row">
						<tr class="${row.index % 2 == 0 ? 'even' : 'odd'}">
							...
							<td width="50px" align="center"><c:choose>
							...
							<td valign="top"><c:choose>
									<c:when test="${task.task == PendingTask.TASK_CHANGE_SECURITY}">
										<table width="100%">
											<tr>
												<td>User:</td>
												<td>${task.params.user}</td>
											</tr>
											<tr>
												<td>Roles:</td>
												<td>${task.params.roles}</td>
											</tr>
											<tr>
												<td>GrantUsers:</td>
												<td>${task.params.grantUsers}</td>
											</tr>
											<tr>
												<td>RevokeUsers:</td>
												<td>${task.params.revokeUsers}</td>
											</tr>
											<tr>
												<td>GrantRoles:</td>
												<td>${task.params.grantRoles}</td>
											</tr>
											<tr>
												<td>RevokeRoles:</td>
												<td>${task.params.revokeRoles}</td>
											</tr>
										</table>
									</c:when>
