	...
	<c:choose>
		<c:when test="${isAdmin}">
			...
			<ul id="breadcrumb">
				...
				<li class="action"><a href="${urlProcessDefinitionView}"> <img
			...
			<table class="results" width="90%">
				...
				<c:forEach var="pi" items="${processInstances}" varStatus="row">
					...
					<tr class="${row.index % 2 == 0 ? 'even' : 'odd'}">
						...
						<td><a href="${urlProcessInstanceView}"><img
								src="img/action/examine.png" alt="Examine" title="Examine" /></a>
							&nbsp; <a class="confirm" href="${urlProcessInstanceDelete}"><img
								src="img/action/delete.png" alt="Delete" title="Delete" /></a> <c:if
								test="${pi.end == null}">
                &nbsp;
                <a href="${urlProcessInstanceEnd}"><img
									src="img/action/end.png" alt="End" title="End" /></a>
							</c:if> &nbsp; <c:choose>
								<c:when test="${pi.suspended}">
									<a href="${urlProcessInstanceResume}"><img
										src="img/action/resume.png" alt="Resume" title="Resume" /></a>
								</c:when>
								<c:otherwise>
									<a href="${urlProcessInstanceSuspend}"><img
			...
			<table class="results" width="90%">
				...
				<c:forEach var="pdf" items="${processDefinitionForms}"
					varStatus="row">
					<tr class="${row.index % 2 == 0 ? 'even' : 'odd'}">
						<td>${pdf.key}</td>
						<td>
