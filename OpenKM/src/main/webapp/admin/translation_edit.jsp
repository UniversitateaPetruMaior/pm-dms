		...
		<c:when test="${isAdmin}">
			...
			<form action="Language" method="post">
				<input type="hidden" name="action" value="${action}" /> <input
					...
					type="hidden" name="lg_id" value="${lg_id}" />
				<table class="results" width="95%">
					...
					<c:forEach var="translation" items="${langBase.translations}"
						varStatus="row">
						<c:choose>
							<c:when
								test="${empty translations[translation.translationId.key]}">
						...
						<tr class="${rowClass}">
							...
							<td width="30%"><input size="60"
								name="${translation.translationId.key}"
								value="${translations[translation.translationId.key]}" /></td>
						</tr>
					</c:forEach>
					<tr class="fuzzy">
						<td colspan="5" align="right"><input type="button"
							onclick="javascript:window.history.back()" value="Cancel"
							class="noButton" /> <input type="submit" value="Edit"
							class="yesButton" /></td>
					</tr>
				</table>
			</form>
