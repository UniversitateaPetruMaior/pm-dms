		...
		<c:when test="${isAdmin}">
			...
			<form action="CronTab" method="post" enctype="multipart/form-data">
				<input type="hidden" name="action" value="${action}" /> <input
					type="hidden" name="ct_id" value="${ct.id}" />
				<table class="form" width="425px">
					<tr>
						<td nowrap="nowrap">Name</td>
						<td><input size="30" class=":required :only_on_blur"
							name="ct_name" value="${ct.name}" /></td>
					</tr>
					<tr>
						<td nowrap="nowrap">Mail</td>
						<td><input size="30" class=":email :only_on_blur"
							name="ct_mail" value="${ct.mail}" /></td>
					</tr>
					<tr>
						<td nowrap="nowrap">Expression</td>
						<td><input class=":required :only_on_blur"
							name="ct_expression" value="${ct.expression}" /></td>
					</tr>
					<tr>
						<td>Active</td>
						<td><c:choose>
