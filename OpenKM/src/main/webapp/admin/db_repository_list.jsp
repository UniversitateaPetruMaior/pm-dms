	...
	<c:choose>
		<c:when test="${isAdmin}">
			...
			<ul id="breadcrumb">
				...
				<li class="action"><a href="${urlRefresh}"> <img
			...
			<table class="results" width="90%">
				...
				<c:forEach var="property" items="${properties}" varStatus="row">
					<tr class="${row.index % 2 == 0 ? 'even' : 'odd'}">
						...
						<td align="center"><c:url value="DbRepositoryView"
								var="urlEdit">
								<c:param name="uuid" value="${node.uuid}" />
							...
							</c:url> <a href="${urlEdit}"><img src="img/action/edit.png"
				...
				<table class="results" width="90%">
					...
					<c:forEach var="child" items="${children}" varStatus="row">
						...
						<tr class="${row.index % 2 == 0 ? 'even' : 'odd'}">
							...
							<td align="center"><c:if test="${child.locked}">
									<img src="img/true.png" />
								</c:if></td>
							<td align="center"><c:if test="${child.checkedOut}">
									<img src="img/true.png" />
								</c:if></td>
			...
			<c:if test="${nodeType == 'document'}">
				...
				<table class="results" width="90%">
					...
					<c:forEach var="version" items="${history}" varStatus="row">
						...
						<tr class="${row.index % 2 == 0 ? 'even' : 'odd'}">
							...
							<td width="120px"><u:formatDate
									calendar="${version.created}" /></td>
							<td>${version.comment}</td>
							<td width="120px" align="center"><a
								href="${urlDownloadVersion}">Download</a> <c:if
									test="${not empty version.previous}">
                  -
				...
				<table class="results" width="90%">
					...
					<tr class="even">
						<c:choose>
							<c:when test="${textExtracted}">
								<td>${text}</td>
							</c:when>
							<c:otherwise>
								<td align="center">Still in text extraction queue</td>
							</c:otherwise>
						</c:choose>
					</tr>
				</table>
