...
<body>
	...
	<c:choose>
		<c:when test="${isAdmin}">
			...
			<ul id="breadcrumb">
				<li class="path"><a
					href="Workflow?action=processDefinitionList">Process
						definitions</a></li>
				<li class="path"><a href="${urlProcessDefinitionView}">Process
						definition</a></li>
				<li class="path">Process instance</li>
				<li class="action"><a href="${urlProcessInstanceView}"> <img
						src="img/action/refresh.png" alt="Refresh" title="Refresh"
			...
			<table class="results" width="90%">
				...
				<tr class="even">
					...
					<td><a href="${urlProcessDefinitionView}">
							${processInstance.processDefinition.name}
							v${processInstance.processDefinition.version} </a></td>
					<td><b> <c:choose>
								<c:when
									test="${processInstance.end != null && processInstance.suspended}">Ended (suspended)</c:when>
								<c:when
									test="${processInstance.end != null && !processInstance.suspended}">Ended</c:when>
								<c:when
									test="${processInstance.end == null && processInstance.suspended}">Suspended</c:when>
								<c:when
									test="${processInstance.end == null && !processInstance.suspended}">Running</c:when>
							</c:choose>
					</b></td>
				...
				<c:forEach var="ti" items="${taskInstances}" varStatus="row">
					...
					<tr class="${row.index % 2 == 0 ? 'even' : 'odd'}">
								...
								<c:otherwise>
									<form id="setActor" action="Workflow">
										<input type="hidden" name="action"
											value="taskInstanceSetActor" /> <input type="hidden"
											name="piid" value="${processInstance.id}" /> <input
											type="hidden" name="tiid" value="${ti.id}" /> <select
											name="actor"
											onchange="document.getElementById('setActor').submit()">
											<option>-</option>
						...
						<td><b> <c:choose>
									...
									<c:when
										test="${ti.end == null && ti.start == null && !ti.suspended}">Not Started</c:when>
									<c:when
										test="${ti.end == null && ti.start == null && ti.suspended}">Not Started (suspended)</c:when>
									<c:when
										test="${ti.end == null && ti.start != null && !ti.suspended}">Running</c:when>
									<c:when
										test="${ti.end == null && ti.start != null && ti.suspended}">Suspended</c:when>
								</c:choose>
						</b></td>
						<td><u:formatDate calendar="${ti.start}" /></td>
						<td><u:formatDate calendar="${ti.end}" /></td>
						<td><a href="${urlTaskInstanceView}"><img
								src="img/action/examine.png" alt="Examine" title="Examine" /></a> <c:if
								test="${!ti.suspended && ti.end == null}">
                &nbsp;
                <a href="${urlTaskInstanceSuspend}"><img
									src="img/action/suspend.png" alt="Suspend" title="Suspend" /></a>
							</c:if> <c:if test="${ti.suspended && ti.end == null}">
                &nbsp;
                <a href="${urlTaskInstanceResume}"><img
									src="img/action/resume.png" alt="Resume" title="Resume" /></a>
							</c:if> <c:if test="${ti.start == null && ti.end == null}">
                &nbsp;
                <a href="${urlTaskInstanceStart}"><img
									src="img/action/start.png" alt="Start" title="Start" /></a>
							</c:if> <c:if
								test="${ti.start != null && ti.end == null && !ti.suspended}">
                &nbsp;
                <a href="${urlTaskInstanceEnd}"><img
									src="img/action/end.png" alt="End" title="End" /></a>
							</c:if></td>
					</tr>
				</c:forEach>
			</table>
			...
			<table class="results" width="90%">
				...
				<c:forEach var="cmt" items="${processInstance.rootToken.comments}"
					varStatus="row">
					<tr class="${row.index % 2 == 0 ? 'even' : 'odd'}">
						<td>${cmt.actorId}</td>
			...
			<form action="Workflow">
				...
				<input type="hidden" name="piid" value="${processInstance.id}" /> <input
					type="hidden" name="tid" value="${processInstance.rootToken.id}" />
				<table class="form">
					<tr>
			...
			<table class="results" width="90%">
				...
				<c:forEach var="tk" items="${processInstance.allTokens}"
					varStatus="row">
					...
					<c:url value="Workflow" var="urlTokenEnd">
						...
						<c:param name="pdid"
							value="${processInstance.processDefinition.id}" />
						<c:param name="piid" value="${processInstance.id}" />
						<c:param name="tid" value="${tk.id}" />
					</c:url>
					...
					<tr class="${row.index % 2 == 0 ? 'even' : 'odd'}">
						...
						<td><a href="${urlTokenView}"><img
								src="img/action/examine.png" alt="Examine" title="Examine" /></a> <c:if
								test="${tk.end == null}">
                &nbsp;
                <a href="${urlTokenEnd}"><img
									src="img/action/end.png" alt="End" title="End" /></a>
							</c:if> <c:choose>
								<c:when test="${tk.suspended}">
                  &nbsp;
                  <a href="${urlTokenResume}"><img
										src="img/action/resume.png" alt="Resume" title="Resume" /></a>
								</c:when>
								<c:otherwise>
                  &nbsp;
                  <a href="${urlTokenSuspend}"><img
										src="img/action/suspend.png" alt="Suspend" title="Suspend" /></a>
								</c:otherwise>
							</c:choose></td>
					</tr>
			...
			<table class="results" width="90%">
				...
				<c:forEach var="var" items="${variables}" varStatus="row">
					...
					<tr class="${row.index % 2 == 0 ? 'even' : 'odd'}">
						...
						<td>${var.value}</td>
						<td><a class="confirm"
							href="${urlProcessInstanceVariableDelete}"><img
								src="img/action/delete.png" alt="Remove" title="Remove" /></a></td>
					</tr>
				</c:forEach>
			</table>
			<br />
			<form action="Workflow">
				<input type="hidden" name="action"
					value="processInstanceVariableAdd" /> <input type="hidden"
					name="piid" value="${processInstance.id}" />
				<table class="form">
					<tr>
