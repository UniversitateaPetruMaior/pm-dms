		...
		<c:when test="${isAdmin}">
			...
			<form action="RepositoryView" method="post">
				<input type="hidden" name="action" value="save" /> <input
					type="hidden" name="path" value="${node.path}" /> <input
					type="hidden" name="property" value="${property.name}" />
				<table class="form" width="350px">
					...
					<c:choose>
						...
						<c:otherwise>
							<tr>
								<td>Value</td>
								<td><input size="64" type="text" name="value"
									value="${value}" /></td>
							</tr>
						</c:otherwise>
					</c:choose>
					<tr>
						<td colspan="2" align="right"><input type="button"
							onclick="javascript:window.history.back()" value="Cancel" /> <input
