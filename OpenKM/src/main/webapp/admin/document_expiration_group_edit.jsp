		...
		<c:when test="${isAdmin}">
			...
			<form action="DocumentExpiration">
				...
				<table class="form" width="250px">
					<tr>
						...
						<c:choose>
							<c:when test="${action == 'groupCreate'}">
								<td><input name="gru_name" class=":required :only_on_blur"
									value="" /></td>
							</c:when>
							<c:otherwise>
					...
					<tr>
						<td>Users</td>
						<td><select multiple="multiple" name="users" size="10">
					...
					<tr>
						<td colspan="2" align="right"><input type="button"
							...
							class="noButton" /> <c:choose>
								...
								<c:when test="${action == 'groupEdit'}">
									<input type="submit" value="Edit" class="yesButton" />
								</c:when>
								<c:when test="${action == 'groupDelete'}">
