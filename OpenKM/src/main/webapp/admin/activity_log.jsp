...
<link rel="stylesheet" type="text/css"
	href="js/jscalendar/calendar-win2k-1.css" />
...
<body>
	...
	<c:choose>
		<c:when test="${isAdmin}">
			<ul id="breadcrumb">
				<li class="path"><a href="ActivityLog">Activity log</a></li>
			</ul>
			<br />
			<form action="ActivityLog">
				<table class="form" align="center">
					<tr>
						<td>From <input type="text" name="dbegin" id="dbegin"
							value="${dbeginFilter}" size="15" readonly="readonly" /> <img
							src="img/action/calendar.png" id="f_trigger_begin"
							style="vertical-align: middle;" /> To <input type="text"
							name="dend" id="dend" value="${dendFilter}" size="15"
							readonly="readonly" /> <img src="img/action/calendar.png"
							id="f_trigger_end" style="vertical-align: middle;" /> User <select
							name="user">
								<option value=""></option>
								...
								<c:forEach var="act" items="${actions}">
									<c:choose>
										<c:when
											test="${act == 'Auth' || act == 'Document' || act == 'Folder' || act == 'Mail' || act == 'Repository' || act == 'Admin' || act == 'Misc'}">
											<optgroup label="${act}" />
										</c:when>
										<c:otherwise>
											<c:choose>
