						...
						<td colspan="2" align="right">
							<form action="LogCat" method="get">
								<input type="hidden" name="action" value="purge" /> <input
									type="submit" value="Purge" class="deleteButton" />
							</form>
						</td>
					</tr>
					<tr>
				...
				<tbody>
					<c:forEach var="log" items="${files}" varStatus="row">
						<tr class="${row.index % 2 == 0 ? 'even' : 'odd'}">
							<td>${log.path}</td>
							<td align="center"><a
								href="LogCat?action=download&file=${log.name}"><img
									src="img/action/compress.png" alt="Download" title="Download" /></a>
								&nbsp; <a href="LogCat?action=view&file=${log.name}"><img
									src="img/action/examine.png" alt="Examine" title="Examine" /></a></td>
						</tr>
					</c:forEach>
				</tbody>
			</table>
