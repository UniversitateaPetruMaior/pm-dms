...
<link rel="stylesheet" type="text/css"
	href="js/codemirror/mode/css/css.css" />
<style type="text/css">
.CodeMirror {
	width: 700px;
	height: 300px;
	...
	<c:choose>
		<c:when test="${isAdmin}">
			<ul id="breadcrumb">
				...
				<li class="path"><a href="Css">CSS list</a></li>
			...
			<form action="Css" method="post" enctype="multipart/form-data">
				<input type="hidden" name="action" value="${action}" /> <input
					...
					type="hidden" name="css_id" value="${css.id}" />
				<table class="form" width="372px">
					<tr>
						<td>Name</td>
						<td width="95%"><input class=":required :only_on_blur"
							name="css_name" value="${css.name}" /></td>
					</tr>
					<tr>
						<td>Context</td>
						<td width="95%"><c:set var="frontend"><%=Css.CONTEXT_FRONTEND%></c:set>
							<c:set var="administration"><%=Css.CONTEXT_ADMINISTRATION%></c:set>
							<c:set var="extension"><%=Css.CONTEXT_EXTENSION%></c:set> <select
							name="css_context">
								<c:choose>
                  ...
                  --%>
					...
					<tr>
						<td colspan="2"><textarea cols="80" rows="25"
								name="css_content" id="css_content">${css.content}</textarea></td>
					</tr>
					<tr>
