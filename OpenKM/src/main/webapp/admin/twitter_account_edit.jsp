	...
	<c:choose>
		<c:when test="${isAdmin}">
			...
			<ul id="breadcrumb">
				...
				<li class="path"><a href="${urlTwitterAccountList}">Twitter
						accounts</a></li>
			...
			<form action="TwitterAccount">
				<input type="hidden" name="action" value="${action}" /> <input
					...
					type="hidden" name="ta_id" value="${ta.id}" /> <input
					type="hidden" name="ta_user" value="${ta.user}" />
				<table class="form" width="330px">
					<tr>
						<td nowrap="nowrap">Twitter user</td>
						<td><input class=":required :only_on_blur" name="ta_tuser"
							value="${ta.twitterUser}" /></td>
					</tr>
					<tr>
						<td>Active</td>
						<td><c:choose>
