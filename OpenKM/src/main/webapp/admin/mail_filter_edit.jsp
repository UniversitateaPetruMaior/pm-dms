		...
		<c:when test="${isAdmin}">
			...
			<ul id="breadcrumb">
				...
				<li class="path"><a href="${urlMailAccountList}">Mail
						accounts</a></li>
				<li class="path"><a href="${urlMailFilterList}">Mail
						filters</a></li>
			...
			<form action="MailAccount" id="form">
				...
				<input type="hidden" name="persist" value="${persist}" /> <input
					type="hidden" name="ma_id" value="${ma_id}" /> <input
					type="hidden" name="ma_user" value="${ma_user}" /> <input
					type="hidden" name="mf_id" value="${mf.id}" />
				<table class="form" width="345px" align="center">
					<tr>
						<td nowrap="nowrap">Folder</td>
						<td><input name="mf_path" id="mf_path" value="${mf.path}"
							size="48" /></td>
						<td><a class="ds"
							href="../extension/DataBrowser?action=repo&sel=fld&dst=mf_path"><img
								src="img/action/browse_repo.png" /></a></td>
					</tr>
					<tr>
						<td>Grouping</td>
						<td><c:choose>
					...
					<tr>
						<td colspan="3" align="right"><input type="button"
							...
							class="noButton" /> <c:choose>
								<c:when test="${action == 'filterCreate'}">
									<input type="submit" value="Create" class="yesButton" />
								</c:when>
								<c:when test="${action == 'filterEdit'}">
									<input type="submit" value="Edit" class="yesButton" />
								</c:when>
								<c:when test="${action == 'filterDelete'}">
