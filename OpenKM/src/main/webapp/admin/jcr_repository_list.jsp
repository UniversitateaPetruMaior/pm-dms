		...
		<c:when test="${isAdmin}">
			...
			<ul>
				...
				<c:if test="${node.depth > 0}">
					<li><b>Actions</b>: <c:url value="RepositoryView"
							var="urlRemoveContent">
							<c:param name="path" value="${node.path}" />
							<c:param name="action" value="remove_content" />
						</c:url> <c:url value="RepositoryView" var="urlRemoveCurrent">
							<c:param name="path" value="${node.path}" />
							<c:param name="action" value="remove_current" />
						</c:url> <a href="${urlRemoveContent}">Remove contents</a> - <a
						href="${urlRemoveCurrent}">Remove current</a> <c:if
							test="${node.locked && holdsLock}">
              ...
              - <a href="${urlCheckin}">Checkin</a>
							...
							test="${managedTextExtraction && (isFolder || isDocument || isDocumentContent) && !node.locked}">
              ...
              - <a href="${urlTextExtraction}">Text Extraction</a>
                  ...
                  - <a href="${urlActivate}">Enable statistics</a>
						...
						</c:if> <c:if test="${isDocument || isFolder}">
						</c:if></li>
			...
			<c:if test="${fn:length(node.mixinNodeTypes) > 0}">
				...
				<ul>
					<c:forEach var="mixin" items="${node.mixinNodeTypes}">
						...
						<li>${fn:toUpperCase(mixin.name)}<a href="${urlRemoveMixin}"><img
								src="img/action/delete.png" alt="Delete" title="Delete" /></a></li>
					</c:forEach>
				</ul>
			</c:if>
			...
			<table class="results" width="90%">
				...
				<c:forEach var="property" items="${properties}" varStatus="row">
					<tr class="${row.index % 2 == 0 ? 'even' : 'odd'}">
						...
						<td align="center"><c:if test="${property.pMultiple}">
								<img src="img/true.png" />
							</c:if></td>
						<td align="center"><c:if test="${property.pProtected}">
								<img src="img/true.png" />
							</c:if></td>
						...
						<td align="center"><c:if
								test="${property.pType == 'STRING' && !property.pProtected}">
