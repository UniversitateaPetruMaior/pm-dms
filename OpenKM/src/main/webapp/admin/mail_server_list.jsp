	...
	<c:choose>
		<c:when test="${isAdmin}">
			...
			<ul id="breadcrumb">
				...
				<li class="action"><a href="${urlServerMails}"> <img
			...
			<table class="results" width="80%">
				<thead>
					<tr class="fuzzy">
						<td colspan="5" align="right">Max: ${max} &nbsp; <c:choose>
								<c:when test="${start > 1}">
									<a
										href="MailAccount?action=serverList&ma_id=${ma_id}&ma_user=${ma_user}&start=1"><img
								...
								<c:when test="${start > 1}">
									<a
										href="MailAccount?action=serverList&ma_id=${ma_id}&ma_user=${ma_user}&start=${start - limit}"><img
								...
								<c:when test="${start + limit < max}">
									<a
										href="MailAccount?action=serverList&ma_id=${ma_id}&ma_user=${ma_user}&start=${max - max % limit + 1}"><img
				...
				<tbody>
					<c:forEach var="sm" items="${serverMails}" varStatus="row">
						...
						<tr class="${row.index % 2 == 0 ? 'even' : 'odd'}"
							...
							<td align="center"><input type="checkbox" name="msg_uid"
								value="${sm.uid}" /></td>
						</tr>
					</c:forEach>
					<tr class="fuzzy">
						<td colspan="5" align="right"><input type="button"
							value="Import" class="addButton" disabled /></td>
					</tr>
				</tbody>
			</table>
