					...
					<tr>
						...
						<td width="100%"><c:choose>
								<c:when test="${action != 'roleCreate'}">
									<input class=":required :only_on_blur" name="rol_id" size="25"
										value="${rol.id}" readonly="readonly" />
								</c:when>
								<c:otherwise>
									<input
										class=":required :only_on_blur :ajax;Auth?action=validateRole"
										name="rol_id" value="" />
								</c:otherwise>
							</c:choose></td>
					</tr>
					...
					<tr>
						<td colspan="2" align="right"><input type="button"
							...
							class="noButton" /> <c:choose>
								...
								<c:when test="${action == 'roleEdit'}">
									<input type="submit" value="Edit" class="yesButton" />
								</c:when>
								<c:when test="${action == 'roleDelete'}">
