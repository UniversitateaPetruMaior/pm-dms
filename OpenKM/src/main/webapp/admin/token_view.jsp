	...
	<c:choose>
		<c:when test="${isAdmin}">
			...
			<ul id="breadcrumb">
				...
				<li class="action"><a href="${urlTokenView}">Refresh</a></li>
			</ul>
			<br />
			<table class="results" width="90%">
				...
				<tr class="even">
					...
					<td><a href="${urlProcessDefinitionView}">
							${token.processInstance.processDefinition.name}
							v${token.processInstance.processDefinition.version} </a></td>
					<td><b> <c:choose>
			...
			<table class="results" width="90%">
				...
				<c:forEach var="tr" items="${token.availableTransitions}"
					varStatus="row">
					...
					<tr class="${row.index % 2 == 0 ? 'even' : 'odd'}">
						...
						<td>${tr.name}</td>
						<td>${tr.to}</td>
						<td><c:if test="${!token.suspended}">
								<a href="${urlTokenSignal}"><img src="img/action/signal.png"
									alt="Signal" title="Signal" /></a>
							</c:if></td>
					</tr>
				</c:forEach>
			</table>
			...
			<table class="results" width="90%">
				...
				<c:forEach var="node"
					items="${token.processInstance.processDefinition.nodes}"
					varStatus="row">
					...
					<tr class="${row.index % 2 == 0 ? 'even' : 'odd'}">
						...
						<td><c:if test="${!token.suspended && token.node != node}">
								<a href="${urlTokenSetNode}"><img src="img/action/move.png"
									alt="Move to this node" title="Move to this node" /></a>
							</c:if></td>
					</tr>
				</c:forEach>
			</table>
