		...
		<c:when test="${isAdmin}">
			...
			<form action="Language" method="post" enctype="multipart/form-data">
				...
				<table class="form" width="372px">
					<tr>
						...
						<td width="100%"><c:choose>
								<c:when test="${action != 'create'}">
									<input size="5" class=":required :only_on_blur" name="lg_id"
										value="${lg.id}" readonly="readonly" />
								</c:when>
								<c:otherwise>
					...
					<tr>
						<td>Name</td>
						<td><input class=":required :only_on_blur" name="lg_name"
							value="${lg.name}" /></td>
					</tr>
					<tr>
						<td>Flag</td>
