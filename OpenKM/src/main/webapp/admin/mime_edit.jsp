		...
		<c:when test="${isAdmin}">
			...
			<form action="MimeType" method="post" enctype="multipart/form-data">
				<input type="hidden" name="action" value="${action}" /> <input
					type="hidden" name="mt_id" value="${mt.id}" />
				<table class="form" width="425px">
					<tr>
						<td>Name</td>
						<td><input class=":required :only_on_blur" size="36"
							name="mt_name" value="${mt.name}" /></td>
					</tr>
					<tr>
						<td>Description</td>
						<td><input class=":required :only_on_blur" size="36"
							name="mt_description" value="${mt.description}" /></td>
					</tr>
					<tr>
						<td>Extensions</td>
						<td><input class=":required :only_on_blur"
							name="mt_extensions" value="${extensions}" /></td>
					</tr>
					<tr>
						<td>Image</td>
						<td><c:choose>
