		...
		<tbody>
			<c:forEach var="fld" items="${folders}">
				...
				<tr class="${row % 2 == 0 ? 'even' : 'odd'}">
					...
					<td align="center"><c:if test="${fld.sel == 'true'}">
							<a href="javascript:void(0)"
								onclick="$('#${dst}', window.parent.document).val('${fld.path}'); window.parent.dialogClose();"><img
								src="img/select.png" alt="Select" title="Select" /></a>
						</c:if></td>
				</tr>
			</c:forEach>
			<c:forEach var="doc" items="${documents}">
				...
				<tr class="${row % 2 == 0 ? 'even' : 'odd'}">
					<td>${doc.name}</td>
					<td align="center"><c:if test="${doc.sel == 'true'}">
							<a href="javascript:void(0)"
								onclick="$('#${dst}', window.parent.document).val('${doc.path}'); window.parent.dialogClose();"><img
								src="img/select.png" alt="Select" title="Select" /></a>
						</c:if></td>
				</tr>
			</c:forEach>
